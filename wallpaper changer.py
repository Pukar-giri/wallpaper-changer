...
#preparing the search string to search for the image in google image search
	...
	# if there is no directory of the search name create one
		if not os.path.exists(rname):
			...
			os.makedirs(rname)
			print("created")
    #creating a flag that would tell us if we have obtained an image or not
		notdone=True
	#until and unless we have obtained an image
		while (notdone):
	#pick a random no between 0 and 50 to try to download that image
			x=randint(0,50)
